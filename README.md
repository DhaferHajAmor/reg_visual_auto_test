...
- Masques exclus : cliquer « Ignorer une zone », tracer UN rectangle (le mode se désactive automatiquement), recliquer pour chaque zone supplémentaire. Redimensionner : poignées carrées blanches sur bords / coins (drag). Une zone ignorée DOIT être entièrement incluse dans une zone de focus s’il en existe (création, déplacement, redimensionnement). La comparaison ignore toujours les zones hors chevauchement. Si les tailles diffèrent, un badge « Tailles des images différentes (A WxH / B WxH) » apparaît (avec dimensions brutes) et une mention dans le statut indique que seule la zone commune est évaluée.
	- Zones de focus : cliquer « Zones focus », tracer UN rectangle (le mode se désactive après ce rectangle), recliquer pour en ajouter un autre. Raccourci rapide : Alt + glisser directement sur l’aire de comparaison (sans activer le bouton) crée immédiatement une zone focus. Redimensionner : survoler une zone focus (poignées carrées vertes), cliquer/drag sur un bord ou un coin pour ajuster. Meta/Ctrl+clic sur une zone pour la retirer. « Effacer focus » supprime toutes les zones et revient au mode global. Quand au moins une zone focus existe, toute nouvelle zone ignorée doit rester à l’intérieur d’au moins une zone focus (création, déplacement, et les masques hors focus sont purgés après ajout ou redimensionnement). À l’ajout d’une zone focus, toutes les zones ignorées qui ne sont pas entièrement incluses dans au moins une zone focus sont automatiquement supprimées (avertissement affiché).
	- Les cadres de zones ignorées et de focus pulsent légèrement pour une meilleure visibilité (animation pure canvas, aucun impact sur le diff). Le message de résultat indique aussi le nombre de zones ignorées actives. (Les zones sont réinitialisées à chaque rechargement de page.)
