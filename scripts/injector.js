...
<div class=\"visdiff-buttons\"><button id=\"vdfit\" class=\"visdiff-btn\">Ajuster au viewport</button><button id=\"vdexport\" class=\"visdiff-btn\">Exporter l\'overlay</button><button id=\"vdcenter\" class=\"visdiff-btn\">Centrer overlay</button><button id=\"vdhide\" class=\"visdiff-btn\">Masquer overlay</button></div>
<div class=\"visdiff-buttons\"><button id=\"vdreset\" class=\"visdiff-btn\">Réinitialiser</button><button id=\"vdclose\" class=\"visdiff-btn danger\">Fermer</button></div>
...
btnCenter.addEventListener('click',()=>{if(img.src){posX=(window.innerWidth/2)-(img.naturalWidth*scale/2)+window.scrollX;posY=(window.innerHeight/2)-(img.naturalHeight*scale/2)+window.scrollY;apply()}});
btnFit.addEventListener('click',()=>{if(img.src){scale=window.innerWidth/img.naturalWidth;scInput.value=String(Math.round(scale*100));apply()}});
